...
open SQLLOG, "query_log_mysql.txt" or die $!;
my @lines = <SQLLOG>;
close SQLLOG;

open SQLOUT, ">query_log_mysql.sql" or die $!;
...
while($i < scalar(@lines)) {
        ...
        if(($current_tokens[1] != 'Query') && ($current_tokens[3] != 'Query')) {
                debug($i, "Not a Query line. Moving on");
                next;
        } else {
                debug($i, "Is a Query line. Parsing for query");
        }

        $query_line = substr($_, index($_, "Query") + 6);
